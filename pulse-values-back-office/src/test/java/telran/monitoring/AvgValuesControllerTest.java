...
void serviceMocking() {
	...
	void allValuesTest() throws UnsupportedEncodingException, Exception {
		String resJSON = mockMvc
				.perform(get("/pulse/values/123").param("from", "2023-01-01T12:00").param("to", "2023-03-01T12:00"))
				.andDo(print()).andExpect(status().isOk()).andReturn().getResponse().getContentAsString();
	...
	void noDatesTest() throws UnsupportedEncodingException, Exception {
		String res = mockMvc.perform(get("/pulse/values/avg/123")).andDo(print()).andExpect(status().isOk()).andReturn()
